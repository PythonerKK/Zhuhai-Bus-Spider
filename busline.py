...
for i in range(1,1000):
    response=requests.get(line_url+str(i),headers=headers)
    ...
    if '{"flag":1004}' in response.text:
        ...
        name=re.compile('"Name":"(\d+)路"').findall(data)
        ...
        csv_in.writerow([id[0],name[0],fromstation[0],tostation[0],begintime[0],endtime[0],count[0]])
        print('%d路公交车数据已写入' % i)
    # print(data)
